...
func (d *Pan115) Link(ctx context.Context, file model.Obj, args model.LinkArgs) (*model.Link, error) {
	...
	if err := d.client.DownloadCreateTicket(file.(*FileObj).PickCode, ticket); err != nil {
		return nil, err
	}
	link := &model.Link{
		URL: ticket.Url,
		Header: http.Header{
			"User-Agent": []string{ticket.Headers["User-Agent"]},
			"Cookie":     []string{ticket.Headers["Cookie"]},
		},
	}
	return link, nil
}
...
func (d *Pan115) Put(ctx context.Context, dstDir model.Obj, stream model.FileStreamer, up driver.UpdateProgress) error {
	...
	/*	var (
			normalTicket = &elevengo.UploadTicket{}
			ossTicket    = &elevengo.UploadOssTicket{}
			dirID        = dstDir.GetID()
		)
